# Fair Weather Rider Lambda v1.3
# Andrew Cargill
# May 28, 2018
...
class HourObject(object):
	...
	def __init__(self, hour, pop, temp):
		self.hour = int(hour)
		self.pop = float(pop) # chance of precipitation
...
def wunderground_hourly():
    url = "http://api.wunderground.com/api/" + API_KEY + "/hourly/q/" + ZIP + ".json"
    response = urllib2.urlopen(url) # Calling Wunderground
    return json.load(response) # .json is read and converted to dictionary

def lambda_handler(event, context):
    weather = wunderground_hourly()
    ...
    for hour in hours:
    	hour.bike_logic() # Invoke each hour object's function to test whether global BIKE_TO_WORK should be toggled off
    	print hour
